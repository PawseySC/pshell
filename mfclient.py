...
# remote_root (the root to prepend to the remote fullpath 
    def copy_fullpath_get(self, from_pattern, from_fullpath, remote_root):
        if from_pattern.endswith('/'):
            pp = PurePath(from_pattern).parent
            relpath = PurePath(from_fullpath).relative_to(pp)
            output = PurePath(remote_root, relpath)
        else:
            fp = PurePath(from_fullpath)
            filename = fp.name
            output = PurePath(remote_root, filename)

# NEW - omit the filename in the output ... this is taken from the source file
#        return str(output)
